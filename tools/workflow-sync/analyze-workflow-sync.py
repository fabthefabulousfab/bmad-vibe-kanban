...
Usage:
    python3 tools/workflow-sync/analyze-workflow-sync.py [OPTIONS]
...
try:
    ...
    print("Install with: pip install -r tools/workflow-sync/requirements.txt")
    sys.exit(1)
...
def load_llm_config(logger: logging.Logger) -> Dict[str, str]:
    ...
    # Load .env file from tools/workflow-sync directory
    script_dir = Path(__file__).parent
    env_path = script_dir / ".env"
    if not env_path.exists():
        logger.error(f".env file not found at {env_path}")
        logger.error("Create .env with: cp tools/workflow-sync/.env.example tools/workflow-sync/.env")
        logger.error("Then configure: BASE_URL, BASE_KEY, BASE_MODEL")
        sys.exit(1)
    ...
    if gitignore_path.exists():
        ...
        if "tools/workflow-sync/.env" not in gitignore_content and ".env" not in gitignore_content:
            logger.error("SECURITY: .env file is NOT in .gitignore!")
            logger.error("Add 'tools/workflow-sync/.env' to .gitignore to prevent credential leaks")
            sys.exit(1)
    else:
